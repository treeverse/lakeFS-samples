...
# MAGIC 
# MAGIC Data Lakehouse is combining the best elements of data lakes and data warehouse into a single platform to assist data teams to operate efficiently. 
# MAGIC With this modern data stack and Lakehouse capabilities, we can enable multiple types of data transformations to co-exist, while eliminating the data silos in data teams. That means better data flows, simpler operational maintenance, and overall better data products!
...
# MAGIC %sql 
# MAGIC 
# MAGIC CREATE TABLE IF NOT EXISTS loan_by_state
# MAGIC USING delta
# MAGIC LOCATION "lakefs://bi-reports/main/tables/loan_by_state/"
...
# MAGIC SELECT * FROM loan_by_state
...
# MAGIC * This notebook will run an `INSERT` every 10s against our `loan_stats` table
...
# MAGIC %sql
# MAGIC -- Review current loans within the `loan_by_state` Delta Lake table
# MAGIC select addr_state, sum(`count`) as loans from loan_by_state group by addr_state

# COMMAND ----------

# MAGIC %md Observe that the Iowa (middle state) has the largest number of loans due to the recent stream of data.  Note that the original `loan_by_state` table is updated as we're reading `loan_by_state_readStream`.
...
# MAGIC DELETE FROM loan_by_state WHERE addr_state = 'IA'
...
# MAGIC UPDATE loan_by_state SET `count` = 2700 WHERE addr_state = 'WA'
...
# MAGIC %sql
# MAGIC MERGE INTO loan_by_state as d
...
# MAGIC %sql
# MAGIC -- Review current loans within the `loan_by_state` Delta Lake table
# MAGIC select addr_state, sum(`amount`) as amount from loan_by_state group by addr_state order by sum(`amount`) desc limit 10
...
# MAGIC %sql
# MAGIC SELECT * FROM loan_by_state VERSION AS OF 0
...
# MAGIC * The following shell statements downloads the us_census_2020 data that we will join with the `loan_by_state` table 
...
# MAGIC * Note that the Delta Lake table is stored in `DELTALAKE_SILVERPATH` or `/ml/loan_by_state`
